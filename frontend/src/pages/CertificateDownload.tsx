...
//     try {
//       setLoading(true);
//       const data = await certificateService.downloadCertificate(courseId!);
...
const CertificateDownload = () => {
  ...
  const fetchCertificate = async () => {
    try {
      setLoading(true);
      const data = await certificateService.downloadCertificate(courseId!);
      setCertificate(data); // 👈 single object
    } catch {
  ...
  if (error) {
    return (
      <div className="p-10 text-red-600 flex flex-col gap-2">
        <Navbar />
        <AlertCircle /> {error}
      </div>
    );
      ...
      <div className="max-w-xl mx-auto p-6">
        <div className="bg-white p-6 rounded shadow text-center">
          ...
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="bg-green-600 text-white px-6 py-2 rounded inline-block mt-4"
          >
